...
public class LoginJFrame extends JFrame implements MouseListener {
	...
	*/
    ...
    static {
        allUsers.add(new User("fafa", "123456"));
        allUsers.add(new User("zhangsan", "456789"));
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        // 点击的是 login 的按钮
        if (obj == login) {
            String usernameInputContent = username.getText();
            String passwordInputContent = password.getText();
            String codeInputContent = code.getText();

            // 判断验证码是否为空
            if (codeInputContent.length() == 0) {
                showJDialog("验证码不能为空...");
                return;
            }

            // 判断账号密码是否为空
            if (usernameInputContent.length() == 0 || passwordInputContent.length() == 0) {
                showJDialog("账号或密码不能为空...");
                return;
            }

            // 判断验证码是否正确
            if (!codeInputContent.equalsIgnoreCase(rightCode.getText())) {
                showJDialog("验证码输入错误");
                return;
            }

            User userInfo = new User(usernameInputContent, passwordInputContent);
