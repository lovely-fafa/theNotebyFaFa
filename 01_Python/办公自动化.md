...
def send_email(subject, content, receivers: List, **kwargs):
    """
	https://learnku.com/articles/51868
    :param subject: 标题
    :param content: 内容
    ...
    mail_user = kwargs['mail_user']  # 用户名
    mail_pass = kwargs['mail_pass']  # 口令
    sender = mail_user
    try:
        ...
        # 必须先登录
        smtpObj.ehlo()  # 用户认证
        # 明文通信协议的扩展，能够让明文的通信连线直接成为加密连线（使用SSL或TLS加密），而不需要使用另一个特别的端口来进行加密通信，属于机会性加密
        smtpObj.starttls()
        smtpObj.login(mail_user, mail_pass)
...
if __name__ == '__main__':
    result = send_email('123', '13', ['', ''], **{
        'mail_user': '',
        'mail_pass': ''
    })
    if result:
        print(f'发送失败 | e={result}')
```
...
todo:https://zhuanlan.zhihu.com/p/518092694
...
1. `filepath`：字符串，PDF文件的文件路径或URL。

2. `pages`：可选参数，字符串，用于指定要提取表格数据的页面范围，可以是逗号分隔的页面号，例如 `'1,3,4'`，也可以使用 `'1,4-end'` 表示从第1页到最后一页，或者 `'all'` 表示提取所有页面。

3. `password`：可选参数，字符串，用于解密PDF文件的密码，如果文件受密码保护。

4. `flavor`：字符串，可选参数，用于指定表格提取的方法。有两个选项：

   一个是基于文本流，一个是基于网格线。比如说有单元格有换行符，那么基于文本流会读成两行，而基于网格线就是一行。
   ...
   - `'lattice'`：用于提取基于网格的表格。
     ...
     - `RuntimeError: Can not find Ghostscript DLL in registry`需要去https://www.ghostscript.com/releases/gsdnld.html安装`DLL`
     - `import ghostscript`

   - `'stream'`：用于提取基于流的表格。

5. `suppress_stdout`：可选参数，布尔值，用于控制是否打印所有日志和警告信息。

6. `layout_kwargs`：可选参数，字典，包含 `pdfminer.layout.LAParams` 的参数，用于控制表格布局的解析。

7. `table_areas`：可选参数，列表，用于指定表格区域的坐标范围，以便提取特定区域的表格数据。

8. `columns`：可选参数，列表，用于指定列的x坐标，以帮助分割表格。

9. `split_text`：可选参数，布尔值，用于控制是否拆分跨越多个单元格的文本。

10. `flag_size`：可选参数，布尔值，用于标记基于字体大小的文本，用于检测上下标。会在标记的文本周围添加 `<s></s>`。

11. `strip_text`：可选参数，字符串，用于指定应从字符串中去除的字符，然后将其分配给单元格。

12. `row_tol`：可选参数，整数，用于控制垂直合并文本的容忍度，以生成行。

13. `column_tol`：可选参数，整数，用于控制水平合并文本的容忍度，以生成列。

14. `process_background`：可选参数，布尔值，用于控制是否处理背景线。

15. `line_scale`：可选参数，整数，用于控制线大小缩放因子，较大的值会使检测到的线条较小。

16. `copy_text`：可选参数，列表，用于指定文本在横跨单元格时的复制方向。

17. `shift_text`：可选参数，列表，用于指定文本在横跨单元格时的流动方向。

18. `line_tol`：可选参数，整数，用于合并接近的垂直和水平线的容忍度。

19. `joint_tol`：可选参数，整数，用于确定检测到的线条和点是否靠近彼此的容忍度。

20. `threshold_blocksize`：可选参数，整数，用于计算像素阈值的像素邻域的大小。

21. `threshold_constant`：可选参数，整数，从均值或加权均值中减去的常数。通常是正数，但也可以是零或负数。

22. `iterations`：可选参数，整数，用于腐蚀/膨胀操作的应用次数。

23. `resolution`：可选参数，整数，用于PDF转换为PNG时的分辨率。

该函数返回一个 `camelot.core.TableList` 对象，其中包含提取的表格数据。



todo：https://zhuanlan.zhihu.com/p/366190934
...
class CT_Anchor(BaseOxmlElement):
    ...
    @classmethod
    def new(cls, cx, cy, shape_id, pic, pos_x, pos_y, is_top):
        """
        Return a new ```` element populated with the values passed
        as parameters.
        """
        anchor = parse_xml(cls._anchor_xml(pos_x, pos_y, is_top))
        anchor.extent.cx = cx
        anchor.extent.cy = cy
    ...
    @classmethod
    def new_pic_anchor(cls, shape_id, rId, filename, cx, cy, pos_x, pos_y, is_top):
        """
        ...
        anchor = cls.new(cx, cy, shape_id, pic, pos_x, pos_y, is_top)
        anchor.graphic.graphicData._insert_pic(pic)
        return anchor

    @classmethod
    def _anchor_xml(cls, pos_x, pos_y, is_top=True):
        return (
                '<wp:anchor distT="0" distB="0" distL="0" distR="0" simplePos="0" relativeHeight="0" \n'
                f'           behindDoc="{0 if is_top else 1}" locked="0" layoutInCell="1" allowOverlap="1" \n'
                '           %s>\n'
...
def new_pic_anchor(part, image_descriptor, width, height, pos_x, pos_y, is_top):
    ...
    return CT_Anchor.new_pic_anchor(shape_id, rId, filename, cx, cy, pos_x, pos_y, is_top)


# refer to docx.text.run.add_picture
def add_float_picture(p, image_path_or_stream, width=None, height=None, pos_x=0, pos_y=0, is_top=True):
    """
    Add float picture at fixed position `pos_x` and `pos_y` to the top-left point of page.
    """
    run = p.add_run()
    anchor = new_pic_anchor(run.part, image_path_or_stream, width, height, pos_x, pos_y, is_top)
    run._r.add_drawing(anchor)
    
    # refer to docx.oxml.__init__.py
    register_element_cls('wp:anchor', CT_Anchor)  # 不知道需不需要？？


if __name__ == '__main__':
    docx = Document('template.docx')  # 创建文件
    paragraph = docx.add_paragraph()

    add_float_picture(paragraph, 'pic.png', width=Inches(1.0), pos_x=Pt(300), pos_y=Pt(200))
...
```python
header = docx.sections[0].header  # 获取第一个节的页眉
paragraph = header.paragraphs[0]  # 获取页眉的第一个段落
add_float_picture(paragraph, 'bg.png', width=Cm(21), pos_x=Pt(0), pos_y=Pt(3), is_top=False)
```

### 1.3 插入页码

https://www.qiniu.com/qfans/qnso-56658872
